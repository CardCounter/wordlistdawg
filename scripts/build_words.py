...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fetch SCOWLv2, filter to lowercase-only entries, normalize to a-z lowercase, and emit words.txt + dict.meta.json."
    )
...
def normalize_words(raw_text: str) -> list[str]:
    ...
    for line in raw_text.splitlines():
        ...
        if re.search(r"[A-Z]", stripped):
            continue
        word = re.sub(r"[^a-z]", "", stripped.lower())
        if word:
            deduped.add(word)
...
def write_meta(
    *,
    ...
    meta["profile"] = {
        ...
        "classes": "core",
        "normalization": "lowercase-only-then-lowercase-alpha-strip",
    }
    meta["stats"] = {
        "wordCount": len(words),
